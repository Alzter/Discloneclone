...
class Agent:
    def __init__(self, use_base_model : bool = False):
        args = LocalModelArguments(
            model_name_or_path = "../models/TestLocal/checkpoint-19000" if not use_base_model else "microsoft/Phi-4-mini-instruct",
            cuda_devices = "0",
...
Do not repeat your previous messages when responding.""".strip().replace("\n", " ")
    ...
    def reply(self, messages : pd.DataFrame, context : str | None = None, max_context_time : timedelta = timedelta(minutes=50), max_lines : int | None = None) -> str:
        """
        ...
        recent_messages = messages[ messages["Date"].apply(lambda x: datetime.now(UTC) - x < max_context_time )]
        ...
        chat_template = parser.to_chat_template(recent_messages, timestamps=True, usernames=True, target_user="alzter", system_prompt=system_prompt)["messages"]
